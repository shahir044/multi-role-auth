...
class AdminCellController extends Controller
{
    ...
    public function index()
    {
        ...
        select('card_requests.id as id', 'approval_status.id as app_id', 'uuid', 'emp_no', 'full_name', 'dept', 'desig', 'mobile', 'receiver', 'stage_name', 'action', 'card_requests.created_at')
            ->where('stage_name','admin-cell')
            ->where('sub_to',\auth()->user()->id)
            ->get();
//        dd($records);
        return view('pages.admin-cell.index', compact('records'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->action == 'reject') {
            ...
            return redirect()->route('admincell-page')->with('success', 'Submission Successfully rejected');
        }
        if ($request->action == 'approve') {
            ...
            try {
                ...
                $approval->stage_name = 'dept-approval'; //next stage after admin-cell
                $approval->sub_to = $request->receiver;
                ...
                return redirect()->route('admincell-page')->with('success', 'Submission Successfully forwarded');
    ...
    public function show(string $id, int $app_id)
    {
        ...
        $form = CardRequest::join('approval_status', 'approval_status.form_no', 'card_requests.id')
            ...
            ->select('card_requests.id', 'uuid', 'type',
                ...
                'approval_status.id as app_id','approval_status.stage_name','approval_status.action','approval_status.remarks')
            ->where('stage_name','admin-cell')
            ->where('sub_to',\auth()->user()->id)
            ->first();

        $approvals = ApprovalStatus::where('id', $app_id)->first();
        $users = User::join('model_has_roles','model_id','users.id')->join('roles','roles.id','role_id')->where('roles.name','=','dept-approval')->orderBy('roles.name')->get(['users.id','users.name','users.email']);
        return view('pages.admin-cell.show', compact('form', 'users', 'app_id', 'approvals'));
    }
